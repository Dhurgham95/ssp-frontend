 ...
 export default {
    ...
    methods : {
      getAllBillsForMerchant() {
        axios.get('http://172.31.71.71:5000/Api/Bill/GetRejectedInsuranceBills', {
      ...
      setTimeout(() => {  
        axios.post('http://172.31.71.71:5000/Api/Bill/GetBillDetails', {
          ...
          this.printedItemDocSeq = response.data.billSeq ,
          this.printedItemAmt = response.data.billAmount ,
          this.printedItemStartDate = response.data.startPayDateForBill.slice(0,10).replace(/-/g,'/'), 
          this.printedItemEndDate = response.data.endPayDateForBill.slice(0,10).replace(/-/g,'/'),
          this.printedItemMonthlyPay = (response.data.billAmount / response.data.payCountForBill).toFixed(3)
          
        ])
      },0 );
        
     //   this.printedItemDocSeq = item.billSeq ; 
     //   this.printedItemAmt = item.billAmount; 
      //  this.printedItemStartDate = item.startPayDateForBill.slice(0,10).replace(/-/g,'/');
        //slice(0,10).replace(/-/g,'/'); 
      //  this.printedItemEndDate = item.endPayDateForBill.slice(0,10).replace(/-/g,'/');
     //   this.printedItemMonthlyPay = item.monthlyPayAmt.toFixed(3);
        // window.print();

        setTimeout(() => {
           axios.post('http://172.31.71.71:5000/Api/Bill/InsuranceApproval', {
         BillSeq : item.billSeq , 
         NoteForRejection : this.notes
       }).then(response => {
         console.log(response);
        }).catch(err => {
         console.log(err)
